...
def init_db():
    with get_conn() as conn:
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS users 
                     (user_id BIGINT PRIMARY KEY, full_name TEXT, username TEXT, join_date TEXT,
                      posted_ads INTEGER DEFAULT 0, paid_slots INTEGER DEFAULT 0, pending_approval INTEGER DEFAULT 0)''')
        c.execute('''CREATE TABLE IF NOT EXISTS settings 
                     (key TEXT PRIMARY KEY, value TEXT)''')
        c.execute('''CREATE TABLE IF NOT EXISTS channels 
                     (id SERIAL PRIMARY KEY, channel_id TEXT, url TEXT)''')
        c.execute('''CREATE TABLE IF NOT EXISTS ads 
                     (id SERIAL PRIMARY KEY, user_id BIGINT, video_id TEXT, text TEXT, status TEXT DEFAULT 'pending')''')
        # UC narxlari
        c.execute('''CREATE TABLE IF NOT EXISTS uc_prices 
                     (id SERIAL PRIMARY KEY, uc_amount INTEGER, price INTEGER, position INTEGER DEFAULT 0)''')
        # UC buyurtmalar
        c.execute('''CREATE TABLE IF NOT EXISTS uc_orders 
                     (id SERIAL PRIMARY KEY, user_id BIGINT, full_name TEXT, username TEXT,
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS stars_prices 
                     (id SERIAL PRIMARY KEY, stars_amount INTEGER, price INTEGER, position INTEGER DEFAULT 0)''')
        # Stars buyurtmalar
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS premium_prices 
                     (id SERIAL PRIMARY KEY, duration TEXT, price INTEGER, position INTEGER DEFAULT 0)''')
        # Premium buyurtmalar
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS unlock_tokens 
                     (token TEXT PRIMARY KEY, used INTEGER DEFAULT 0, created BIGINT)''')

        # Default sozlamalar
        c.execute("INSERT INTO settings (key, value) VALUES ('price', '50000') ON CONFLICT (key) DO NOTHING")
        c.execute("INSERT INTO settings (key, value) VALUES ('card', '8600 0000 0000 0000 (Ism Familiya)') ON CONFLICT (key) DO NOTHING")
        c.execute("INSERT INTO settings (key, value) VALUES ('start_msg', 'Salom {name}! Siz bu botdan PUBG Mobile akkauntingizni obzorini joylashingiz mumkin va u video kanalga joylanadi.') ON CONFLICT (key) DO NOTHING")
        c.execute("INSERT INTO settings (key, value) VALUES ('site_url', 'https://azizbekqiyomov55555-dev.github.io/Test-bot-') ON CONFLICT (key) DO NOTHING")
        conn.commit()

def db_query(query, params=(), fetchone=False, fetchall=False):
    """
    SQLite sintaksisidagi '?' ni PostgreSQL '%s' ga almashtiradi va query bajaradi.
    """
    ...
    with get_conn() as conn:
        ...
        if fetchall:
            return c.fetchall()
        conn.commit()
        # RETURNING id ishlatmagan holdagi INSERT uchun lastrowid
        try:
            return c.fetchone()[0] if c.rowcount > 0 else None
        except:
            return None

def db_insert(query, params=()):
    """INSERT qilib yangi yozuvning id sini qaytaradi"""
    pg_query = query.replace("?", "%s")
    # RETURNING id qo'shamiz agar yo'q bo'lsa
...
async def uc_reject_cb(call: CallbackQuery):
    ...
    await bot.send_message(user_id,
        ...
        "Iltimos, qayta urinib ko'ring yoki 🆘 Yordam orqali admin bilan bog'laning.",
        parse_mode="HTML", reply_markup=get_main_menu())
    caption = call.message.caption or ""
    try:
...
async def get_level(message: Message, state: FSMContext):
    ...
    await message.answer("Nechta qurol (upgradable) bor? Kiriting:")
...
async def get_phone(message: Message, state: FSMContext):
    ...
    ad_id = db_insert("INSERT INTO ads (user_id, video_id, text) VALUES (?, ?, ?)",
...
async def admin_panel(message: Message):
    btn = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [
            ...
            InlineKeyboardButton(text="💳 Karta o'zgartirish", callback_data="admin_card"),
        ],
        ...
        [
            InlineKeyboardButton(text="📋 UC narxlari / o'chirish", callback_data="admin_uc_list"),
        ...
        [
            InlineKeyboardButton(text="📋 Stars narxlari / o'chirish", callback_data="admin_stars_list"),
        ...
        [
            InlineKeyboardButton(text="📋 Premium narxlari / o'chirish", callback_data="admin_premium_list"),
            InlineKeyboardButton(text="📦 Premium buyurtmalar", callback_data="admin_premium_orders"),
        ],
    ])
    await message.answer("⚙️ <b>Admin panel</b>", reply_markup=btn, parse_mode="HTML")
...
async def add_uc_price_step1(call: CallbackQuery, state: FSMContext):
    await call.message.answer(
        "💎 <b>UC miqdorini kiriting</b>\n\nMasalan: <code>60</code> yoki <code>300</code>",
        parse_mode="HTML"
    )
...
async def admin_uc_list(call: CallbackQuery):
    ...
    for pid, uc_amount, price in prices:
        ...
        rows.append([InlineKeyboardButton(text=f"🗑 {uc_amount} UC o'chirish", callback_data=f"del_uc_price_{pid}")])
...
async def del_uc_price(call: CallbackQuery):
    ...
    row = db_query("SELECT uc_amount FROM uc_prices WHERE id=?", (pid,), fetchone=True)
    if row:
        db_query("DELETE FROM uc_prices WHERE id=?", (pid,))
        await call.answer(f"✅ {row[0]} UC narxi o'chirildi!", show_alert=True)
        try:
            await call.message.delete()
        except:
            pass
...
async def admin_stars_list(call: CallbackQuery):
    ...
    for pid, stars_amount, price in prices:
        ...
        rows.append([InlineKeyboardButton(text=f"🗑 {stars_amount} Stars o'chirish", callback_data=f"del_stars_price_{pid}")])
    rows.append([InlineKeyboardButton(text="🔙 Admin panel", callback_data="back_to_admin")])
    await call.message.answer(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
    await call.answer()
...
async def del_stars_price(call: CallbackQuery):
    pid = int(call.data.split("_")[3])
    row = db_query("SELECT stars_amount FROM stars_prices WHERE id=?", (pid,), fetchone=True)
    if row:
        db_query("DELETE FROM stars_prices WHERE id=?", (pid,))
        await call.answer(f"✅ {row[0]} Stars narxi o'chirildi!", show_alert=True)
        try:
            await call.message.delete()
        except:
            pass
...
async def add_premium_price_step1(call: CallbackQuery, state: FSMContext):
    await call.message.answer(
        "⭐ <b>Premium muddatini kiriting</b>\n\nMasalan: <code>1 oylik</code> yoki <code>3 oylik</code>",
        parse_mode="HTML"
    )
...
async def admin_premium_list(call: CallbackQuery):
    ...
    for pid, duration, price in prices:
        ...
        rows.append([InlineKeyboardButton(text=f"🗑 {duration} o'chirish", callback_data=f"del_premium_price_{pid}")])
    rows.append([InlineKeyboardButton(text="🔙 Admin panel", callback_data="back_to_admin")])
    await call.message.answer(text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=rows))
    await call.answer()
...
async def del_premium_price(call: CallbackQuery):
    ...
    if row:
        ...
        await call.answer(f"✅ {row[0]} narxi o'chirildi!", show_alert=True)
        try:
            await call.message.delete()
        except:
            pass
...
def generate_stats_image():
    ...
    uc_orders_count = db_query("SELECT COUNT(*) FROM uc_orders", fetchone=True)[0]
    uc_approved = db_query("SELECT COUNT(*) FROM uc_orders WHERE status='approved'", fetchone=True)[0]
    stars_orders_count = db_query("SELECT COUNT(*) FROM stars_orders", fetchone=True)[0]
    premium_orders_count = db_query("SELECT COUNT(*) FROM premium_orders", fetchone=True)[0]
    show_users = (users or [])[:30]
...
async def check_bot_status(message: Message):
    ...
    try:
        ...
        await message.answer(f"❌ Xatolik: {e}\n\nBotni {MAIN_CHANNEL_ID} kanaliga admin sifatida qo'shing!")

# ================== ASOSIY ISHGA TUSHIRISH ==================
async def main():
    dp.include_router(router)
    print("✅ Bot ishga tushdi (PostgreSQL)...")
