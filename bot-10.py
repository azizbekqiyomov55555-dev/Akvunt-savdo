...
async def jb_read() -> dict:
    """Jsonbin dan barcha ma'lumotlarni o'qish"""
...
async def jb_write(data: dict) -> bool:
    """Jsonbin ga ma'lumotlarni yozish"""
...
# ================== ASOSIY MENU (RANGLI TUGMALAR) ==================
# Bot API 9.4+: style='primary' (ko'k), 'success' (yashil), 'danger' (qizil/to'q sariq)
def get_main_menu():
    kb = ReplyKeyboardMarkup(
        keyboard=[
            ...
            [
                KeyboardButton(text="🎮 PUBG MOBILE UC OLISH 💎", style="success")  # 🟢 Yashil
            ],
            [
                KeyboardButton(text="⭐ TELEGRAM PREMIUM", style="danger"),   # 🔴 Qizil
                KeyboardButton(text="🌟 STARS OLISH", style="primary")        # 🔵 Ko'k
            ],
        ],
...
async def approve_ad(call: CallbackQuery):
    ...
    # Har bir e'lon uchun avtomatik rang: primary, success, danger navbat bilan
...
async def admin_clear_uc_ask(call: CallbackQuery):
    ...
    await call.message.answer("⚠️ <b>Barcha UC narxlarini o'chirasizmi?</b>\nBu amalni qaytarib bo'lmaydi!", parse_mode="HTML", reply_markup=btn)
    await call.answer()
...
async def admin_clear_stars_ask(call: CallbackQuery):
    ...
    await call.message.answer("⚠️ <b>Barcha Stars narxlarini o'chirasizmi?</b>\nBu amalni qaytarib bo'lmaydi!", parse_mode="HTML", reply_markup=btn)
    await call.answer()
...
async def admin_clear_premium_ask(call: CallbackQuery):
    ...
    await call.message.answer("⚠️ <b>Barcha Premium narxlarini o'chirasizmi?</b>\nBu amalni qaytarib bo'lmaydi!", parse_mode="HTML", reply_markup=btn)
    await call.answer()
...
async def add_uc_price_step1(call: CallbackQuery, state: FSMContext):
    await call.message.answer("💎 <b>UC miqdorini kiriting</b>\n\nMasalan: <code>60</code>", parse_mode="HTML")
...
async def del_uc_price(call: CallbackQuery):
    ...
    if item:
        ...
        if not new_prices:
            try:
                await call.message.edit_text("❌ Barcha UC narxlari o'chirildi.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="🔙 Admin panel", callback_data="back_to_admin", style="primary")]
                ]))
            except:
                pass
            return
...
async def add_stars_price_step1(call: CallbackQuery, state: FSMContext):
    await call.message.answer("⭐ <b>Stars miqdorini kiriting</b>\n\nMasalan: <code>50</code>", parse_mode="HTML")
...
async def del_stars_price(call: CallbackQuery):
    ...
    if item:
        ...
        if not new_prices:
            try:
                await call.message.edit_text("❌ Barcha Stars narxlari o'chirildi.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="🔙 Admin panel", callback_data="back_to_admin", style="primary")]
                ]))
            except:
                pass
...
async def add_premium_price_step1(call: CallbackQuery, state: FSMContext):
    await call.message.answer("⭐ <b>Premium muddatini kiriting</b>\n\nMasalan: <code>1 oylik</code>", parse_mode="HTML")
...
async def del_premium_price(call: CallbackQuery):
    ...
    if item:
        ...
        if not new_prices:
            try:
                await call.message.edit_text("❌ Barcha Premium narxlari o'chirildi.", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="🔙 Admin panel", callback_data="back_to_admin", style="primary")]
                ]))
            except:
                pass
